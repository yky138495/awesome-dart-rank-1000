...
def handle_request(h_url,p):
    ...
    while retry_count > 0:
        try:
            response_b= ''
            header = {'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/50.0.2661.102 Safari/537.36','Connection': 'keep-alive'}
            proxies = {
...
#                                'https': "https://{}".format(proxy)
            ...
            print(proxies)
            if enable_proxy:
                response_b =requests.get(url, headers=header,proxies=proxies, timeout=20)
                print(url)
            else:
